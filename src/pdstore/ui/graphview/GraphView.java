...
public class GraphView extends JPanel implements MouseListener,
	...
	public GraphView(PDStore store, Object... instances) {
		...
		for (Object instance : instances) {
			...
			node.addInstance((GUID) instance);
	...
	void initUI() {
		...
		addNewInstanceMenuItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Query query = new Query(select, where, null, null, null);
				Iterator<Map<Variable, Object>> resultSet = query.execute(copy
						.getTransaction());

				List<Object> typeNames = new ArrayList<Object>();
				while (resultSet.hasNext()) {
					Map<Variable, Object> result = resultSet.next();
					Debug.println(
							"Found type name: " + result.get(typeNameVar),
							"addNewInstance");
				...
				if ((typeName != null) && (typeName.length() > 0)) {
					...
					newNode.addInstance(newInstanceID);
		...
		removeNodeMenuItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (PDNode node : selectedNodes) {
					nodeLabels += copy.getLabel(node.getInstance()) + "\n";
				}
	...
	public void paint(Graphics g) {
		...
		for (PDNode node1 : graph.getNodes()) {
			Point location1 = new Point(node1.getX().intValue(), (int) node1
					.getY().intValue());
			GUID instance1 = node1.getInstance();
			GUID type1 = store.getType(transaction, node1.getInstance());
			...
			for (GUID role2 : store.getAccessibleRoles(transaction, type1)) {
				...
				for (Object instance2 : store.getInstances(transaction,
					...
					// draw the edge
					g2d.drawLine(location1.x, location1.y, location2.x,
							location2.y);
				}
			}
		}
		...
		for (PDNode node : graph.getNodes()) {
			...
			g2d.fill(new Ellipse2D.Double(location.x - radius, location.y
					- radius, 2 * radius, 2 * radius));

			// draw the label
			String label = copy.getLabel(node.getInstance());
			FontMetrics metrics = g2d.getFontMetrics();
			int height = metrics.getHeight();
			int width = metrics.stringWidth(label);
			g2d.drawString(label, location.x - width / 2, (int) (location.y
					- radius - 5));
		}
