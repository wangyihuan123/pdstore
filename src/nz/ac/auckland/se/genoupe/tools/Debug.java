...
public class Debug {
	...
	/**
	 * Names of the classes that should be debugged.
	 */
	...
	public Set<String> getDebugClasses() {
		return debugClasses;
	}

	public void setDebugClasses(Set<String> debugClasses) {
	...
	public static void debugThisClass(Object... debugTopics) {
		StackTraceElement elem = err.getCaller();
		err.output.println("Debugging: " + elem);

		err.debugClasses.add(elem.getClassName());

		setMaxDebugging(getMaxDebugging() + 1000);
		addDebugTopic(debugTopics);
	}
	...
	private static boolean reachedDebugLimitForClasses() {
		return maxDebugging != 0 && currentDebugging > maxDebugging;
	}
	...
	public static boolean isDebugging(int depth, Object... debugTopics) {
		...
		// check if one of the given topics is being debugged
		boolean debuggingTopic = isDebuggingTopic(debugTopics);
		if (err.isGlobalDebug() || debuggingTopic)
			return true;

		// allow to limit how often we call class-based debugging.
		if (reachedDebugLimitForClasses())
			return false;
		++currentDebugging;

		// check for class-based debugging
		StackTraceElement elem = err.getCaller(depth);

		// write a final warning if Debug is exhausted
		if (maxDebugging != 0 && currentDebugging - 1 == maxDebugging) {
			err.outputDebugInfo(
					elem,
					"\n!!!Maximum Debug for Classes Exhausted!!!\n!!!No further Debug Output for Classes!!!");
			return false;
		}

		return err.isDebugging(elem);
	}
	...
	public boolean isDebugging(StackTraceElement elem) {
		return debugClasses.contains(elem.getClassName());
	}
